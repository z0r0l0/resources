...
### 变更
- **顶层结构收敛**：把「主机配置」统一收进 `system/`，顶层由 8 个目录精简为
  6 个**纯角色化**目录，消除「同为配置却平级」的歧义
  - `git-config/` → `system/git/`
  - `hermes/` → `system/hermes/`
- 顶层现按「角色」正交划分：`system/`（怎么配）· `scripts/`（怎么跑）·
  `knowledge/`（怎么读）· `templates/`（复制什么）· `tools/`（怎么起步）
- README 目录树为每个目录补充角色标注；文档索引补入 Git / Hermes 配置说明
